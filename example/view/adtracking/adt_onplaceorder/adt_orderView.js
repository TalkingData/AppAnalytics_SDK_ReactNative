...
import {TalkingDataADTOrder, TalkingDataAdTracking} from '../../../TalkingDataAdTracking'
...
export class ADTOnPlaceOrderScreen extends React.Component {
                                ...
                                () => {
                                    if (this.state.orderId != "" && this.state.total != "") {
                                        if (RegexUtil.isNum(this.state.total)){
                                            this.state.order = new TalkingDataADTOrder(this.state.orderId, this.state.total, this.state.currencyType);
                                            alert("生成订单成功")
                                        }else{
                                            alert("订单总价必须为数字")
                                        }
                                        ...
                                        if (this.state.item.itemId !== ""){
                                            ...
                                            if (RegexUtil.isNum(this.state.item.unitPrice)) {
                                                if (RegexUtil.isNaturalNum(this.state.item.amount)) {
                                                    this.state.order.addItemWithItemId(this.state.item.itemId, this.state.item.category, this.state.item.name, this.state.item.unitPrice, this.state.item.amount);
                                                    alert("添加物品成功")
                                                }else {
                                                    alert("物品数量必须为非负数")
                                                }
                            ...
                            onPress={
                                () => {
                                    if (this.state.accountId != "") {
                                        if (this.state.order != null) {
                                            TalkingDataAdTracking.onPlaceOrder(this.state.accountId, this.state.order.orderString);
                                            alert("调用订单接口成功")
                                        } else {
                                            alert("请先生成订单！");
                                        }
