...
/**
  ...
  @"TYPE10":自定义类型10,
*/
const TDACCOUNT = NativeModules.TDAccountType;
...
class TalkingDataAppAnalytics {
	...
	*/
	static onRegister(accountId,accountType,name){
		TD.onRegister(accountId,accountType,name);
	}
	...
	* @param {string} name 账户昵称
	*/
	static onLogin(accountId,accountType,name){
		TD.onLogin(accountId,accountType,name);
	}
	...
	*/
	static onPlaceOrder(accountId,order){
		TD.onPlaceOrder(accountId,order);
	}

	static onOrderPaySucc(accountId,payType,order){
		TD.onOrderPaySucc(accountId,payType,order);
	}
...
export {TalkingDataAppAnalytics,TDACCOUNT,TalkingDataOrder,TalkingDataShoppingCart};
